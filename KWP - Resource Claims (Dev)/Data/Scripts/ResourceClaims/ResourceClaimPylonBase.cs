...
namespace Khjin.ResourceClaims
{
    public abstract class ResourceClaimPylonBase : IMyEventProxy
    {
        ...
        // Production
        /// <summary>
        /// Base ore produced by miner every 100 ticks.
        /// </summary>
        protected float _baseOreAmount;
        /// <summary>
        /// Flat offset from the center of the block downwards. Added with _miningZoneRadius to get the center point of mining.
        /// </summary>
        ...
        protected MinedOres _minedOres;
        /// <summary>
        /// Radius at which the current pylon affects allied pylons.
        /// </summary>
        protected float _interferenceZoneRadius;
        /// <summary>
        /// Radius at which the current pylon affects enemy pylons.
        /// </summary>
        protected float _suppressionZoneRadius;
        /// <summary>
        /// Amount of yield percentage caused by the pylon against enemy pylons.
        /// </summary>
        protected float _suppressionPenaltyFactor;
        /// <summary>
        /// Amount of yield percentage caused by the pylon against ally pylons.
        /// </summary>
        ...
        // Synced properties
        private MinedOres _lastSynchedMinedOres;
        ...
        private void SynchronizeStatus()
        {
            ...
            if (_lastSynchedMinedOres != _minedOres)
            {
                _lastSynchedMinedOres = _minedOres;
                _logic._synchedMinedOres.Value = _minedOres;
            }
        }
        ...
        private void GetOresAt(Vector3D worldPosition, MyVoxelBase voxelMap)
        {
            ...
            if (materialId != 0 && materialId != 255)
            {
                ...
                {
                    // TODO: Think of a better way to deal with Ice (Ignore for now).
                    if (!_detectedOres.ContainsKey(definition.MinedOre))
                    {
                        ...
                        _minedOres |= ToMinedOreEnum(definition.MinedOre);
                    }
                }
            }
        }
        ...
        private void CalculateEffectiveness()
        {
            ...
            if (_resourcePylon.UpgradeValues.ContainsKey("Effectiveness"))
            {
                ...
                {
                    // Reduce the bonus effectiveness by half
                    float bonus = (_effectiveness - 1.0f) * 0.5f;
                    _effectiveness = 1.0f + bonus;
                }
            }
            else
            { _effectiveness = 1.0f; }
            ...
            {
                if (otherPylon != this && !otherPylon.IsProjection())
                {
        ...
        {
            var faction = MyAPIGateway.Session.Factions.TryGetPlayerFaction(_resourcePylon.OwnerId);
            List<IMyPlayer> players = new List<IMyPlayer>();
            if (faction != null)
            {
                MyAPIGateway.Players.GetPlayers(players, p => faction.Members.ContainsKey(p.IdentityId));
                foreach (var onlineMember in players)
                {
                    var combatMessage = new CombatMessage()
                    {
                        Message = $"Resource claim {_resourcePylon.CustomName} is under attack!",
                        Color = "Red",
                        Recipient = onlineMember.IdentityId
                    };

                    if (!_combatMessages.Contains(combatMessage))
                    {
                        _combatMessages.Enqueue(combatMessage);
                    }
                }
            }
        }
        ...
        public void ProcessWarnings()
        {
            ...
            if (_combatMessages.Count > 0 && _combatMessages.TryDequeue(out message))
            {
                MessagePlayer(message.Message, message.Recipient, message.Color);
            }
        }
        ...
        private MinedOres ToMinedOreEnum(string minedOreName)
        {
            switch(minedOreName)
            {
                ...
                case "Uranium":     return MinedOres.Uranium;
                case "Platinum":    return MinedOres.Platinum;
                case "Ice":         return MinedOres.Ice;
        ...
        private void SaveDetectedOres()
        {
            ...
            if (logic != null)
            {
                ...
                foreach(var oreProfile in _detectedOres.Values)
                {
                    currentOres += oreProfile.MaterialId.ToString() + ":";
                }
                string saveString = $"{currentLocation.Trim()}>{currentOres.TrimEnd(':')}";
                logic.OresData = saveString;
            }
        }
        ...
        private bool LoadSavedDetectedOres()
        {
            ...
            string[] oreIds = savedDectedOres.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            {
                byte materialId;
                if (byte.TryParse(oreId, out materialId))
                {
                    var definition = MyDefinitionManager.Static.GetVoxelMaterialDefinition(materialId);
                    if (definition != null)
                    {
        ...
        {
            MyCubeGrid cubeGrid = _resourcePylon.CubeGrid as MyCubeGrid;
            return cubeGrid != null && (cubeGrid.IsPreview || cubeGrid.Projector != null);
        }
        ...
        public MinedOres MinedOres
        {
            get
            {
                ...
                {
                    return _logic._synchedMinedOres == null ? MinedOres.Unknown : _logic._synchedMinedOres; 
                }
            }
        }
    }
